...
public class ProcessController {
    ...
    public String completeTask(
        ...
        try {
            return JSON.toJSONString(processServiceImpl.completeTask(taskId, modelId, Assignee, nextNode, comformInfo,comformId, businessKey, opinion, serialNumber, urgencyDegree), SerializerFeature.WriteMapNullValue);
        } catch (CustomException ce) {
            return JsonUtil.error(ce.getMessage());
        }
    }
    ...
    @ResponseBody
    public String getprocessImageDate(@ApiParam(value = "任务ID", required = false) @RequestParam(value = "taskId", required = false) String taskId) {
        try {
            Map map = processServiceImpl.getprocessImageDate(taskId);
